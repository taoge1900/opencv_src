// opencv_contrib-4.10.0/modules/tracking/src/trackerCSRT.cpp
...
// ===== TrackerCSRTV2 Implementation =====

/**
 * @brief Enhanced TrackerCSRT implementation with PSR score access
 */
class TrackerCSRTV2Impl CV_FINAL : public TrackerCSRTV2
{
public:
    TrackerCSRTV2Impl(const Params &parameters = Params());
    virtual ~TrackerCSRTV2Impl() CV_OVERRIDE;
    ...
    // TrackerCSRTV2 specific methods
    virtual double getTrackingScore() const CV_OVERRIDE;
    virtual double getRawPSR() const CV_OVERRIDE;
    virtual bool isTargetLost() const CV_OVERRIDE;
    virtual TrackingStats getTrackingStats() const CV_OVERRIDE;

private:
    // Internal CSRT implementation with PSR access
    Ptr<TrackerCSRTImpl> csrt_impl_;
    
    // Statistics tracking
    mutable std::deque<double> psr_history_;
    mutable int successful_frames_;
    mutable int total_frames_;
    static constexpr size_t MAX_HISTORY_SIZE = 100;
    
    // Internal methods
    void updateTrackingStatistics(double psr, bool success) const;
    double normalizeTrackingScore(double psr) const;
};

TrackerCSRTV2Impl::TrackerCSRTV2Impl(const TrackerCSRT::Params &parameters)
    : successful_frames_(0)
    , total_frames_(0)
{
    csrt_impl_ = makePtr<TrackerCSRTImpl>(parameters);
}

TrackerCSRTV2Impl::~TrackerCSRTV2Impl()
{
    // Destructor
}

void TrackerCSRTV2Impl::init(InputArray image, const Rect& boundingBox)
{
    csrt_impl_->init(image, boundingBox);
    
    // Initialize statistics
    successful_frames_ = 1;
    total_frames_ = 1;
    psr_history_.clear();
    psr_history_.push_back(1.0);
}

bool TrackerCSRTV2Impl::update(InputArray image, Rect& boundingBox)
{
    // Call the original update method (which now updates PSR internally)
    bool success = csrt_impl_->update(image, boundingBox);
    
    total_frames_++;
    
    if (success) {
        successful_frames_++;
        // Get real PSR value from internal implementation
        double psr_value = csrt_impl_->getLastPSRValue();
        updateTrackingStatistics(psr_value, true);
    } else {
        // Target lost
        updateTrackingStatistics(0.0, false);
    }
    
    return success;
}

void TrackerCSRTV2Impl::setInitialMask(InputArray mask)
{
    csrt_impl_->setInitialMask(mask);
}

double TrackerCSRTV2Impl::getTrackingScore() const
{
    double psr = csrt_impl_->getLastPSRValue();
    return normalizeTrackingScore(psr);
}

double TrackerCSRTV2Impl::getRawPSR() const
{
    return csrt_impl_->getLastPSRValue();
}

bool TrackerCSRTV2Impl::isTargetLost() const
{
    return csrt_impl_->getTargetLostStatus();
}

TrackerCSRTV2::TrackingStats TrackerCSRTV2Impl::getTrackingStats() const
{
    TrackingStats stats;
    stats.current_psr = csrt_impl_->getLastPSRValue();
    stats.successful_frames = successful_frames_;
    stats.total_frames = total_frames_;
    stats.success_rate = total_frames_ > 0 ? 
        static_cast<double>(successful_frames_) / total_frames_ : 0.0;
    
    if (!psr_history_.empty()) {
        double sum = 0.0;
        for (double psr : psr_history_) {
            sum += psr;
        }
        stats.avg_psr = sum / psr_history_.size();
        
        auto minmax = std::minmax_element(psr_history_.begin(), psr_history_.end());
        stats.min_psr = *minmax.first;
        stats.max_psr = *minmax.second;
    } else {
        stats.avg_psr = stats.min_psr = stats.max_psr = 0.0;
    }
    
    return stats;
}

double TrackerCSRTV2Impl::normalizeTrackingScore(double psr) const
{
    TrackerCSRTImpl* impl = static_cast<TrackerCSRTImpl*>(csrt_impl_.get());
    double threshold = impl->params.psr_threshold;
    ...
    double normalized = 1.0 / (1.0 + std::exp(-10.0 * (psr - threshold)));
    return std::max(0.0, std::min(1.0, normalized));
}

void TrackerCSRTV2Impl::updateTrackingStatistics(double psr, bool success) const
{
    psr_history_.push_back(psr);
    if (psr_history_.size() > MAX_HISTORY_SIZE) {
        psr_history_.pop_front();
    }
}

// TrackerCSRTV2 public interface
TrackerCSRTV2::TrackerCSRTV2()
{
    // nothing
}
...
Ptr<TrackerCSRTV2> TrackerCSRTV2::create(const TrackerCSRT::Params &parameters)
{
    return makePtr<TrackerCSRTV2Impl>(parameters);
}
